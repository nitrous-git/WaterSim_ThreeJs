...
export class ScreenSpaceFluidRenderer {
    ...
    createCompositeMaterial() {
        return new THREE.ShaderMaterial({
            uniforms: {
                ...
                uShallowColor: {
                    value: new THREE.Color(0x55ccff)
                },

                uDeepColor: {
                    value: new THREE.Color(0x06395f)
                },

                uLightDirection: {
                    value: new THREE.Vector3(0.5, 1.0, 0.35).normalize()
                },
    ...
    allocateRenderTargets() {
        ...
        this.depthTarget = this.createFluidDepthTarget(
            targetWidth,
            targetHeight,
            true
        );

        this.blurTargetA = this.createFluidDepthTarget(
            targetWidth,
            targetHeight,
            false
        );

        this.blurTargetB = this.createFluidDepthTarget(
            targetWidth,
            targetHeight,
            false
        );
    ...
    render(renderer, scene, camera) {
        ...
        this.depthMaterial.uniforms.uPointRadius.value =
            this.particleRadius;

        this.depthMaterial.uniforms.uPointScale.value =
            this.targetHeight *
            camera.projectionMatrix.elements[5];

        this.depthMaterial.uniforms.uProjectionMatrix.value.copy(
            camera.projectionMatrix
        );

        this.compositeMaterial.uniforms.uProjectionMatrixInverse.value.copy(
            camera.projectionMatrixInverse
        );

        this.compositeMaterial.uniforms.uCameraNear.value =
            camera.near;

        this.compositeMaterial.uniforms.uCameraFar.value =
            camera.far;
        ...
        this.compositeMaterial.uniforms.uLightDirection.value.copy(
            this.lightDirectionView
        );
        ...
        for (let i = 0; i < this.blurIterations; i++) {
            this.blurMaterial.uniforms.uDepthTexture.value =
                depthTexture;
        ...
        // --------------------------------------------------------
        // Pass 4: composite water surface over the scene
        // --------------------------------------------------------
        ...
        this.compositeMaterial.uniforms.uFluidDepth.value =
            depthTexture;
